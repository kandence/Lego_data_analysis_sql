...
select * from [portfolio2].[dbo].[sets];
select * from [portfolio2].[dbo].[inventories];
select * from [portfolio2].[dbo].[minifigs];
select * from [portfolio2].[dbo].[inventory_minifigs];
select * from [portfolio2].[dbo].[themes]
...
as Century
from  [portfolio2].[dbo].[sets] s
left join [portfolio2].[dbo].[themes] t
	on s.theme_id = t.id
left join [portfolio2].[dbo].[themes] p
	on t.parent_id = p.id
GO
...
select sum(total_set_num) Total_trains_themed, sum(percentage) Percentage_trained_themed
from(
...
from 
	(
		...
		from [portfolio2].[dbo].[inventory_parts] inv
		inner join [portfolio2].[dbo].[colors] c
			on inv.color_id = c.id
		inner join [portfolio2].[dbo].[parts] p
			on inv.part_num = p.part_num
		inner join [portfolio2].[dbo].[part_categories] pc
